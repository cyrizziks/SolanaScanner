...
// Jupiter V6 (quote-api.jup.ag/v6) was deprecated Oct 1 2025. Current free,
// keyless endpoint is the Metis quote API on lite-api. (Higher limits: swap to
// api.jup.ag/swap/v1/quote with an `x-api-key` header from portal.jup.ag.)
  ...
  try {
    ...
    for (const raw of probes) {
      ...
      const url = `${QUOTE}?inputMint=${mint}&outputMint=${WSOL}&amount=${amount}`
        + `&slippageBps=50&swapMode=ExactIn&restrictIntermediateTokens=true`;
      try {
