...
async function analyze(ca, endpoint, depth, onProgress) {
  ...
  report.metrics = { top10, top1, freshCount, cexCount, biggestCluster, tokenAgeDays, flaggedCount, serialCount };
          ...
          {r.holders.map((h, i) => (
            <button key={i} onClick={() => onWallet(h)}
              ...
              <div style={{ display: "flex", gap: 5, flexWrap: "wrap", justifyContent: "flex-end", width: 200, alignItems: "center" }}>
                ...
                {(h.tags || []).slice(0, 2).map((t, j) => <Tag key={j} t={t} />)}
              </div>
  ...
  const persist = async (next) => {
    ...
    try {
      const res = await fetch(INTEL, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ op: "label", address: h.owner, manual_tags: merged.manualTags, note: merged.note, flagged: merged.flagged }),
      });
    ...
    <div onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 40, background: "rgba(0,0,0,.5)", display: "flex", justifyContent: "flex-end" }}>
      <div onClick={(e) => e.stopPropagation()} className="rise"
        ...
        <div style={{ marginTop: 20, display: "grid", gap: 1, background: "#1b1d22", border: "1px solid #1b1d22", borderRadius: 10, overflow: "hidden" }}>
          ...
          <Row k="Realized PnL" v="next phase" muted />
        </div>
...
function Tag({ t, big }) {
  const map = { "FLAGGED": "#ff5d5d", "Fresh": "#ffb454", "Sniper?": "#ff5d5d", "Whale": "#ff8fb1", "CEX-funded": "#5b8def", "High-activity": "#a78bfa" };
