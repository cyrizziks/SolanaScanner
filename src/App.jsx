...
const LIQUIDITY = "/.netlify/functions/liquidity";        // (#1) Jupiter exit-slippage
const FLOW = "/.netlify/functions/flow";                  // (#2 + #6) flow + authenticity
const DEPLOYER_HISTORY = "/.netlify/functions/deployer-history"; // (#4) prior launches
const PNL_CACHE = {}; // address -> result, cached for the session to avoid re-spending credits
const PORTFOLIO_CACHE = {}; // address -> portfolio value, cached for the session
const DHIST_CACHE = {}; // deployer address -> prior-launch history, cached for the session
...
const money = (x) => x == null ? "—" : x >= 1000 ? "$" + Intl.NumberFormat("en", { notation: "compact", maximumFractionDigits: 2 }).format(x) : x >= 1 ? "$" + x.toFixed(2) : "$" + Number(x).toPrecision(3);
...
async function analyze(ca, endpoint, holdersN, traceDepth, onProgress) {
  ...
  // 1b) Price, market cap, and metadata mutability (Helius DAS getAsset)
  step("Pricing token…");
  try {
    const asset = await rpc("getAsset", { id: ca, displayOptions: { showFungible: true } });
    const price = asset?.token_info?.price_info?.price_per_token ?? null;
    report.token.price = price;
    report.token.fdv = price != null ? price * supply : null;
    report.token.mutable = !!asset?.mutable;
    report.token.name = asset?.content?.metadata?.name || null;
    report.token.symbol = asset?.token_info?.symbol || asset?.content?.metadata?.symbol || null;
  } catch { /* price/metadata unavailable (often the case for brand-new tokens) */ }
  ...
  holders = holders.map((h) => {
    ...
    if (!known && id) {
      ...
      known = { label: id.name || id.category || "Labeled", kind: k || "label" };
      kind = k;
    }
  ...
  try {
    ...
    if (res.ok) {
      ...
      holders = holders.map((h) => {
        const r = map[h.owner];
        if (!r) return h;
        const manualTags = r.manual_tags || [];
        const pnlSol = typeof r.pnl_sol === "number" ? r.pnl_sol : Number(r.pnl_sol) || 0;
        const smartMoney = (!!r.pnl_seen && pnlSol >= 5) || manualTags.includes("good trader") || manualTags.includes("established") || !!r.early_buyer;
        return { ...h, intel: {
          ...
          pnlSol, pnlSeen: !!r.pnl_seen, earlyBuyer: !!r.early_buyer, smartMoney,
        } };
      });
    }
  ...
  const smartCount = holders.filter((h) => !h.isPool && h.intel?.smartMoney).length;
  ...
  report.metrics = { top10, top1, freshCount, cexCount, biggestCluster, freshClusterPct, freshClusterCount, cexClusterPct, otherClusterPct, tokenAgeDays, flaggedCount, serialCount, smartCount, deployerPct, equalCount, maliciousCount };
  ...
  if (report.token.mutable) report.flags.push({ t: "Token metadata is mutable — dev can change name / symbol / socials", sev: "med" });
  ...
  if (smartCount) report.flags.push({ t: `${smartCount} known smart-money wallet(s) holding (profitable / established in your intel)`, sev: "good" });

  // (#1) Exit liquidity via Jupiter — probe selling fractions of supply.
  //      Sized by SUPPLY FRACTION so it works even when price is unknown.
  step("Probing exit liquidity (Jupiter)…");
  try {
    const dec = report.token.decimals || 0;
    const fracs = [0.001, 0.0025, 0.005, 0.01];
    const probes = fracs.map((f) => Math.floor(report.token.supply * f * 10 ** dec)).filter((x) => x > 0);
    if (probes.length) {
      const res = await authedFetch(LIQUIDITY, { mint: ca, probes });
      if (res.ok) {
        const data = await res.json();
        report.liquidity = { routable: !!data.routable, probes: (data.probes || []).map((p, i) => ({ ...p, frac: fracs[i] })) };
        if (data.routable === false) report.flags.push({ t: "No Jupiter exit route found — likely pre-migration or no real liquidity", sev: "high" });
      }
    }
  } catch { /* liquidity probe offline — scan still completes */ }

  // (#2 + #6) Recent buy/sell flow + trade authenticity (token-level swaps).
  step("Reading recent flow…");
  try {
    const res = await authedFetch(FLOW, { mint: ca });
    if (res.ok) report.flow = await res.json();
  } catch { /* flow offline */ }
  if (report.flow && report.flow.sampled > 20) {
    const f = report.flow;
    if (f.uniqueTraders && f.sampled && f.uniqueTraders / f.sampled < 0.35)
      report.flags.push({ t: `Low trader diversity — ${f.uniqueTraders} wallets across ${f.sampled} swaps (possible wash)`, sev: "med" });
    if (f.netSol < 0 && Math.abs(f.netSol) > 5)
      report.flags.push({ t: `Net distribution: ${f.netSol.toFixed(1)} ◎ more sold than bought in recent swaps`, sev: "med" });
  }

  // (#5) Bundle overhang — supply STILL HELD by coordinated/bundle wallets =
  //      a pending sell wall. Conservative: the larger of the two bundle signals.
  const overhangPct = Math.max(report.metrics.freshClusterPct || 0, report.equalCluster?.pct || 0);
  const overhangSources = [];
  if ((report.metrics.freshClusterPct || 0) > 0) overhangSources.push(`${report.metrics.freshClusterCount} fresh cluster(s)`);
  if ((report.equalCluster?.pct || 0) > 0) overhangSources.push(`${report.equalCluster.count} equal-balance wallets`);
  report.metrics.overhangPct = overhangPct;
  report.overhang = overhangPct > 0 ? { pct: overhangPct, sources: overhangSources } : null;
  if (overhangPct >= 5) report.flags.push({ t: `Bundle overhang: ${pct(overhangPct)} still held by ${overhangSources.join(" + ")} — pending sell wall`, sev: overhangPct >= 12 ? "high" : "med" });
...
function classify(h, tokenAgeDays) {
  const tags = [];
  if (h.known?.label) tags.push(h.known.label);
...
function scoreToken(r) {
  ...
  if (t.mutable) add("Mutable metadata", -6);
  ...
  // (#1) exit liquidity — can you actually get out at size?
  if (r.liquidity) {
    if (!r.liquidity.routable) add("No Jupiter exit route", -25);
    else {
      const mid = (r.liquidity.probes || []).find((p) => p.frac === 0.005 && p.routable) || (r.liquidity.probes || []).find((p) => p.routable);
      if (mid && mid.impactPct != null)
        add(`Exit impact ~${mid.impactPct.toFixed(1)}% @${(mid.frac * 100).toFixed(2)}% supply`, mid.impactPct > 30 ? -22 : mid.impactPct > 15 ? -12 : mid.impactPct > 6 ? -4 : +6);
    }
  }
  // (#5) bundle overhang still held
  if ((m.overhangPct || 0) >= 5) add(`Bundle overhang ${pct(m.overhangPct)} held`, -Math.min(20, Math.round(m.overhangPct)));
  ...
  render() {
    ...
    return (
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: 20 }}>
        <div style={{ background: "#1a1012", border: "1px solid #ff5d5d55", borderRadius: 14, padding: 20, color: "#ffb4b4" }}>
          <div className="display" style={{ fontWeight: 700, fontSize: 16, color: "#ff5d5d" }}>Render error</div>
          <div style={{ marginTop: 8, fontSize: 13, lineHeight: 1.5 }}>The scan finished but something broke while drawing the results. Copy this and send it to me — it names the exact cause.</div>
          <pre style={{ marginTop: 12, whiteSpace: "pre-wrap", wordBreak: "break-word", fontSize: 11, color: "#e7c4c4", background: "#0a0b0d", border: "1px solid #2a2d34", borderRadius: 8, padding: 12, maxHeight: 280, overflow: "auto" }}>{String((e && e.message) || e)}{"\n\n"}{String((e && e.stack) || "").split("\n").slice(0, 8).join("\n")}</pre>
        </div>
      </div>
    );
  }
...
export default function App() {
  ...
  // Fetch a wallet that isn't in the holder list (e.g. a funder) on demand and
  // open it in the drawer. Lets you drill funder -> funder.
  const [inspecting, setInspecting] = useState(false);
  const inspectWallet = useCallback(async (address) => {
    if (!address || !report || inspecting) return;
    setInspecting(true);
    try {
      const rpc = makeRpc(RPC_PROXY);
      const supply = report.token?.supply || 0;
      const idMap = await fetchIdentities([address]);
      const id = idMap[address] || null;
      let amount = 0;
      try {
        const r = await rpc("getTokenAccountsByOwner", [address, { mint: report.ca }, { encoding: "jsonParsed" }]);
        amount = (r?.value || []).reduce((s, a) => s + (a.data?.parsed?.info?.tokenAmount?.uiAmount || 0), 0);
      } catch { /* holds none of this token */ }
      let ageDays = null, txCount = null, funder = null, firstSeen = null;
      try {
        const sigs = await rpc("getSignaturesForAddress", [address, { limit: 1000 }]);
        if (sigs?.length) {
          txCount = sigs.length;
          const oldest = sigs[sigs.length - 1];
          firstSeen = oldest.blockTime; ageDays = daysAgo(oldest.blockTime);
          const tx = await rpc("getTransaction", [oldest.signature, { maxSupportedTransactionVersion: 0, encoding: "jsonParsed" }]);
          const ix = tx?.transaction?.message?.instructions || [];
          const transfer = ix.find((x) => x.parsed?.type === "transfer" && x.parsed?.info?.destination === address);
          const k0 = tx?.transaction?.message?.accountKeys?.[0];
          funder = transfer?.parsed?.info?.source || (typeof k0 === "object" ? k0?.pubkey : k0);
        }
      } catch { /* age/funder unavailable */ }
      let intel;
      try {
        const res = await authedFetch(INTEL, { op: "sync", token: report.ca, wallets: [{ address, tags: [] }] });
        if (res.ok) { const rows = await res.json(); const rr = (Array.isArray(rows) ? rows : []).find((x) => x.address === address); if (rr) intel = { tokensSeen: (rr.tokens_seen || []).length, manualTags: rr.manual_tags || [], note: rr.note || "", flagged: !!rr.flagged, lastSeen: rr.last_seen }; }
      } catch { /* intel offline */ }
      const k = id ? (categoryKind(id.category) || "label") : null;
      const known = id ? { label: id.name || id.category || "Labeled", kind: k } : null;
      const h = { owner: address, amount, pctSupply: supply ? (amount / supply) * 100 : 0, identity: id, known, isPool: k === "lp" || k === "burn", malicious: k === "malicious", ageDays, txCount, funder, firstSeen, intel };
      h.tags = classify(h, report.metrics?.tokenAgeDays);
      setDrawer(h);
    } catch { /* ignore */ }
    setInspecting(false);
  }, [report, inspecting]);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0a0b0d", color: "#e7e3d8", fontFamily: "'IBM Plex Mono', ui-monospace, monospace" }}>
      ...
      <ErrorBoundary key={report?.ca || "none"}>
        {report && <Report r={report} onWallet={setDrawer} copy={copy} copied={copied} />}
        {drawer && <Drawer key={drawer.owner} h={drawer} onClose={() => setDrawer(null)} copy={copy} copied={copied} onUpdate={updateIntel} onInspect={inspectWallet} />}
      </ErrorBoundary>
      {!report && !loading && <Empty />}

      {inspecting && (
        <div style={{ position: "fixed", inset: 0, zIndex: 60, background: "rgba(0,0,0,.55)", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, background: "#101216", border: "1px solid #2a2d34", borderRadius: 12, padding: "14px 20px", color: "#d4ff47", fontSize: 13 }}>
            <Loader2 size={16} style={{ animation: "spin 1s linear infinite" }} /> Inspecting wallet…
          </div>
        </div>
      )}
        ...
        <div style={{ background: "#0e1014", border: "1px solid #1b1d22", borderRadius: 14, padding: 20 }}>
          {(r.token.name || r.token.symbol) && (
            <div className="display" style={{ fontSize: 18, fontWeight: 700, marginBottom: 8 }}>
              {r.token.name || r.token.symbol}{r.token.name && r.token.symbol ? <span style={{ color: "#6b6f78", fontWeight: 600 }}> · {r.token.symbol}</span> : null}
            </div>
          )}
          ...
          <div style={{ display: "flex", gap: 22, marginTop: 18, flexWrap: "wrap" }}>
            {r.token.fdv != null && <Stat label="MKT CAP" value={money(r.token.fdv)} />}
            {r.token.price != null && <Stat label="PRICE" value={money(r.token.price)} />}
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 18, flexWrap: "wrap" }}>
            {r.token.mutable && (
              <span style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: 11, color: "#ffb454", background: "rgba(255,180,84,.1)", border: "1px solid rgba(255,180,84,.3)", borderRadius: 7, padding: "5px 9px" }}>Mutable metadata</span>
            )}
          ...
          {r.deployer?.address && <DeployerHistory deployer={r.deployer.address} ca={r.ca} />}
        </div>
        {/* confidence gauge */}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, alignItems: "start" }}>
        ...
        <div className="rise" style={{ background: "#0e1014", border: "1px solid #1b1d22", borderRadius: 14, padding: 18 }}>
          ...
          <div style={{ marginTop: 12, padding: 10, background: "#0a0b0d", borderRadius: 8, fontSize: 11, color: "#6b6f78", lineHeight: 1.5 }}>
            <Sparkles size={12} style={{ verticalAlign: "-2px" }} /> Score = organic + sellability signals (incl. exit liquidity & bundle overhang). Flow, authenticity & smart money read out below — they answer "is it going somewhere," kept separate on purpose.
          </div>
        </div>
      </div>
          ...
          {r.holders.map((h, i) => (
            <button key={i} onClick={() => onWallet(h)}
              ...
              <div style={{ display: "flex", gap: 5, flexWrap: "wrap", justifyContent: "flex-end", width: 200, alignItems: "center" }}>
                ...
                {h.intel?.smartMoney && !h.intel?.flagged && (
                  <span style={{ fontSize: 10, color: "#7ee787", border: "1px solid #7ee78744", background: "#7ee78714", borderRadius: 6, padding: "2px 7px", whiteSpace: "nowrap" }}>SMART $</span>
                )}
...
function Drawer({ h, onClose, copy, copied, onUpdate, onInspect }) {
  ...
  const [portfolio, setPortfolio] = useState(PORTFOLIO_CACHE[h.owner] || null);
  ...
  const computePnl = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        PNL_CACHE[h.owner] = data; setPnl(data);
        // (#3) persist as smart-money reputation so it resurfaces on every future scan
        if (typeof data.netSol === "number") {
          const early = (h.tags || []).includes("Sniper?") || (h.tags || []).includes("Fresh");
          try { await authedFetch(INTEL, { op: "pnl", address: h.owner, net_sol: data.netSol, early }); } catch { /* intel offline */ }
        }
  ...
  const computePortfolio = async () => {
    ...
    try {
      const rpc = makeRpc(RPC_PROXY);
      const r = await rpc("getAssetsByOwner", { ownerAddress: h.owner, page: 1, limit: 1000, displayOptions: { showFungible: true, showNativeBalance: true } });
      const items = r?.items || [];
      let tokenVal = 0, withPrice = 0;
      items.forEach((a) => { const tp = a.token_info?.price_info?.total_price; if (typeof tp === "number") { tokenVal += tp; withPrice++; } });
      const solVal = r?.nativeBalance?.total_price || 0;
      const data = { total: tokenVal + solVal, solVal, tokens: withPrice, holdings: items.length };
      PORTFOLIO_CACHE[h.owner] = data; setPortfolio(data);
    } catch { setPortfolio({ error: true }); }
    setPfBusy(false);
  };
    ...
    <div onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 40, background: "rgba(0,0,0,.5)", display: "flex", justifyContent: "flex-end" }}>
      <div onClick={(e) => e.stopPropagation()} className="rise"
        ...
        <div style={{ marginTop: 20, display: "grid", gap: 1, background: "#1b1d22", border: "1px solid #1b1d22", borderRadius: 10, overflow: "hidden" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "11px 13px", background: "#0e1014" }}>
            <span style={{ fontSize: 12, color: "#8a8f99" }}>Funded by</span>
            {h.funder ? (
              <button onClick={() => onInspect && onInspect(h.funder)}
                style={{ background: "transparent", border: "none", color: "#d4ff47", fontSize: 12, fontFamily: "inherit", display: "inline-flex", alignItems: "center", gap: 5, cursor: "pointer", padding: 0 }}>
                {short(h.funder)} <ChevronRight size={13} />
              </button>
            ) : <span style={{ fontSize: 12, color: "#6b6f78" }}>—</span>}
          </div>
          <Row k="Seen in your scans" v={(intel.tokensSeen || 0) + " token(s)"} />
          {intel.pnlSeen && <Row k="Stored PnL (last read)" v={(Number(intel.pnlSol) >= 0 ? "+" : "") + Number(intel.pnlSol || 0).toFixed(2) + " ◎"} />}
          {intel.smartMoney && <Row k="Reputation" v="Smart money ✓" />}
        ...
        {/* net worth — on demand, costs Helius credits */}
        <div style={{ marginTop: 12, padding: 14, background: "#0a0b0d", border: "1px solid #1b1d22", borderRadius: 10 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <Wallet size={14} color="#d4ff47" />
            <span className="display" style={{ fontWeight: 600, fontSize: 13 }}>Net worth</span>
            <span style={{ fontSize: 10, color: "#6b6f78", marginLeft: "auto" }}>portfolio · USD</span>
          </div>

          {!portfolio && (
            <button onClick={computePortfolio} disabled={pfBusy}
              style={{ marginTop: 12, width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 7, padding: "10px", borderRadius: 8, fontSize: 12, fontWeight: 600, border: "1px solid #2a2d34", background: "transparent", color: pfBusy ? "#6b6f78" : "#d4ff47" }}>
              {pfBusy ? <Loader2 size={14} style={{ animation: "spin 1s linear infinite" }} /> : <Wallet size={14} />} {pfBusy ? "Reading holdings…" : "Estimate net worth (uses credits)"}
            </button>
          )}

          {portfolio?.error && <div style={{ marginTop: 12, fontSize: 12, color: "#ff5d5d" }}>Couldn't read this wallet's holdings.</div>}

          {portfolio && typeof portfolio.total === "number" && (
            <div style={{ marginTop: 12 }}>
              <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
                <span className="display" style={{ fontSize: 30, fontWeight: 800, color: portfolio.total > 50000 ? "#7ee787" : "#e7e3d8" }}>{money(portfolio.total)}</span>
                <span style={{ fontSize: 11, color: "#6b6f78" }}>{portfolio.tokens} priced tokens · incl. SOL</span>
              </div>
              <div style={{ marginTop: 8, fontSize: 11, color: "#6b6f78", lineHeight: 1.5 }}>
                Total value of priced holdings + SOL. Established, diversified wallets are a stronger organic signal than fresh empties. Unpriced/illiquid tokens aren't counted.
              </div>
              {portfolio.total > 50000 && !(intel.manualTags || []).includes("established") && (
                <button onClick={() => persist({ manualTags: [...new Set([...(intel.manualTags || []), "established"])] })}
                  style={{ marginTop: 10, width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 7, padding: "9px", borderRadius: 8, fontSize: 12, fontWeight: 600, border: "1px solid rgba(126,231,135,.4)", background: "rgba(126,231,135,.12)", color: "#7ee787" }}>
                  <ShieldCheck size={14} /> Tag as established
                </button>
              )}
            </div>
          )}
...
/* ===== Tradeability — exit liquidity (#1) + flow/authenticity (#2/#6) + smart
   money (#3). Kept SEPARATE from the organic score on purpose: this answers
   "can I get out, and is it going somewhere," not "is it organic." ===== */
function Tradeability({ r }) {
  const liq = r.liquidity, flow = r.flow, m = r.metrics;
  if (!liq && !flow && !(m && m.smartCount)) return null;
  ...
  const impColor = (ip) => ip == null ? "#6b6f78" : ip > 15 ? "#ff5d5d" : ip > 6 ? "#ffb454" : "#7ee787";
  const buy = flow?.buySol || 0, sell = flow?.sellSol || 0, tot = buy + sell;
  const washy = flow && flow.sampled > 20 && flow.uniqueTraders / flow.sampled < 0.35;

  return (
    <div className="rise" style={{ background: "#0e1014", border: "1px solid #1b1d22", borderRadius: 14, padding: 18, marginTop: 14 }}>
      <Head icon={Activity} title="Tradeability" sub="can you exit · is it going somewhere" />

      {/* (#1) exit liquidity */}
      <div style={{ marginTop: 12 }}>
        <div style={{ fontSize: 11, color: "#6b6f78", letterSpacing: ".08em", marginBottom: 8 }}>EXIT LIQUIDITY · sell into SOL via Jupiter</div>
        {!liq && <div style={{ fontSize: 12, color: "#6b6f78" }}>Not probed.</div>}
        {liq && !liq.routable && <div style={{ fontSize: 12, color: "#ff5d5d" }}>No Jupiter route — likely pre-migration or no real liquidity. Treat as unsellable until it migrates.</div>}
        {liq && liq.routable && (liq.probes || []).map((p, i) => (
          <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "6px 0", borderBottom: i < liq.probes.length - 1 ? "1px solid #16181d" : "none" }}>
            <span style={{ width: 90, fontSize: 12, color: "#9aa0ab" }}>sell {(p.frac * 100).toFixed(2)}%</span>
            {p.routable ? (
              <>
                <span style={{ fontSize: 12, color: "#e7e3d8", width: 88 }}>{p.outSol != null ? p.outSol.toFixed(2) + " ◎" : "—"}</span>
                <span style={{ flex: 1, fontSize: 12, fontWeight: 600, color: impColor(p.impactPct) }}>{p.impactPct != null ? p.impactPct.toFixed(1) + "% impact" : "—"}</span>
                <span style={{ fontSize: 10, color: "#6b6f78", maxWidth: 130, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{(p.venues || []).join(", ")}</span>
              </>
            ) : <span style={{ flex: 1, fontSize: 12, color: "#ff5d5d" }}>no route at this size</span>}
          </div>
        ))}
        <div style={{ fontSize: 9, color: "#4a4d54", marginTop: 4 }}>% = share of total supply sold in one go. Impact = Jupiter price impact across all routes.</div>
      </div>

      {/* (#2 + #6) flow + authenticity */}
      <div style={{ marginTop: 18 }}>
        <div style={{ fontSize: 11, color: "#6b6f78", letterSpacing: ".08em", marginBottom: 8 }}>RECENT FLOW · {flow ? `last ${flow.sampled} swaps ${fmtSpan(flow.spanStart, flow.spanEnd)}${flow.capped ? " · sampled" : ""}` : "—"}</div>
        {!flow && <div style={{ fontSize: 12, color: "#6b6f78" }}>Not read.</div>}
        {flow && (
          <>
            <div style={{ display: "flex", height: 8, borderRadius: 99, overflow: "hidden", background: "#16181d" }}>
              <div style={{ width: tot ? `${(buy / tot) * 100}%` : "0%", background: "#7ee787" }} title="buys" />
              <div style={{ width: tot ? `${(sell / tot) * 100}%` : "0%", background: "#ff5d5d" }} title="sells" />
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: 8, fontSize: 12 }}>
              <span style={{ color: "#7ee787" }}>Buy {buy.toFixed(1)} ◎</span>
              <span style={{ color: (flow.netSol || 0) >= 0 ? "#7ee787" : "#ff5d5d" }}>net {(flow.netSol || 0) >= 0 ? "+" : ""}{(flow.netSol || 0).toFixed(1)} ◎</span>
              <span style={{ color: "#ff5d5d" }}>Sell {sell.toFixed(1)} ◎</span>
            </div>
            <div style={{ display: "flex", gap: 16, marginTop: 10, fontSize: 11, color: "#8a8f99", flexWrap: "wrap" }}>
              <span>{flow.uniqueTraders} unique traders / {flow.sampled} swaps</span>
              <span style={{ color: washy ? "#ffb454" : "#8a8f99" }}>{flow.roundTrips} round-trips{washy ? " · low diversity" : ""}</span>
            </div>
          </>
        )}
      </div>
      ...
      {/* (#3) smart money */}
      <div style={{ marginTop: 18, paddingTop: 14, borderTop: "1px solid #16181d", display: "flex", alignItems: "center", gap: 8 }}>
        <ShieldCheck size={14} color={m && m.smartCount ? "#7ee787" : "#6b6f78"} />
        <span style={{ fontSize: 12, color: m && m.smartCount ? "#7ee787" : "#6b6f78" }}>
          {m && m.smartCount ? `${m.smartCount} known smart-money wallet(s) in this token` : "No smart-money wallets from your intel here yet — compute PnL / tag traders in the drawer to build it up"}
        </span>
      </div>
    </div>
  );
...
/* ===== Deployer launch history (#4) — on-demand. Has this dev shipped before,
   and did those tokens die? One serial-rugger hit outweighs many heuristics. ===== */
function DeployerHistory({ deployer, ca }) {
  const [data, setData] = useState(DHIST_CACHE[deployer] || null);
  const [busy, setBusy] = useState(false);
  const run = async () => {
    if (busy) return;
    setBusy(true);
    try {
      const res = await authedFetch(DEPLOYER_HISTORY, { deployer, exclude: ca });
      if (res.ok) { const d = await res.json(); DHIST_CACHE[deployer] = d; setData(d); }
      else setData({ error: true });
    } catch { setData({ error: true }); }
    setBusy(false);
  };
  const serial = data && !data.error && data.priorCount >= 1;
  const ruggy = data && !data.error && data.deadCount >= 2;
  return (
    <div style={{ marginTop: 8 }}>
      {!data && (
        <button onClick={run} disabled={busy}
          style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "transparent", border: "1px solid #2a2d34", color: busy ? "#6b6f78" : "#d4ff47", borderRadius: 8, padding: "6px 10px", fontSize: 11 }}>
          {busy ? <Loader2 size={12} style={{ animation: "spin 1s linear infinite" }} /> : <Boxes size={12} />} {busy ? "Scanning prior launches…" : "Check deployer's other launches (uses credits)"}
        </button>
      )}
      {data?.error && <div style={{ fontSize: 11, color: "#ff5d5d" }}>Couldn't read deployer history.</div>}
      {data && !data.error && (
        <div style={{ fontSize: 12, color: ruggy ? "#ff5d5d" : serial ? "#ffb454" : "#7ee787", lineHeight: 1.5 }}>
          {data.priorCount === 0
            ? "No other launches found in the sampled history — looks like a first launch."
            : `${data.priorCount} other launch(es) found${data.capped ? " (sampled)" : ""} · ${data.deadCount} look dead/rugged.`}
          {data.priorCount > 0 && (
            <div style={{ marginTop: 6, display: "flex", flexDirection: "column", gap: 3 }}>
              {data.priorMints.slice(0, 6).map((p, i) => (
                <span key={i} style={{ fontSize: 11, color: p.dead ? "#ff5d5d" : "#9aa0ab" }}>
                  {p.dead ? "✗" : "•"} {p.name || short(p.mint)}{p.dead ? " — dead" : (p.price != null ? ` — $${Number(p.price).toPrecision(2)}` : "")}
                </span>
              ))}
            </div>
          )}
